...
import { Plus, Minus, DollarSign, ArrowLeft } from 'lucide-react';
...
export default function EditInvoice({ params }) {
  ...
  useEffect(() => {
    if (status === 'unauthenticated') {
      router.push('/login');
    } else if (session?.user?.role && !['admin', 'staff'].includes(session.user.role)) {
  ...
  useEffect(() => {
    const fetchInvoice = async () => {
      try {
        ...
        const formattedClientPayments = invoice.clientPayments.map(payment => ({
          ...payment,
        ...
        reset({
          ...
          workOrderId: invoice.workOrder._id,
          clientPayments: formattedClientPayments,
  ...
  useEffect(() => {
    const totalClientPayment = watchClientPayments?.reduce((sum, payment) => sum + (Number(payment.amount) || 0), 0) || 0;
  ...
  useEffect(() => {
    const fetchWorkOrderDetails = async (id) => {
      try {
        ...
        // Try to find the work order in the already loaded array
    ...
    if (watchWorkOrderId && watchWorkOrderId !== selectedWorkOrder?._id) {
      fetchWorkOrderDetails(watchWorkOrderId);
    }
  }, [watchWorkOrderId, workOrders, selectedWorkOrder]);
  ...
  return (
    <DashboardLayout title="Edit Invoice">
      <div className="max-w-5xl mx-auto">
        <div className="mb-6">
          <Link href={`/invoices/${id}`} className="flex items-center text-blue-600 hover:text-blue-800">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {/* Work Order */}
            <div className="mb-4">
              <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor="workOrderId">
                Work Order *
              </label>
              <select
